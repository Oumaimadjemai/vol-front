...
import { 
  ...
  FaPassport, FaCreditCard, FaHistory, FaFileAlt, FaLock, 
  FaSave, FaEdit, FaSignOutAlt, FaPlane, FaCheckCircle,
  FaTimesCircle, FaClock, FaPrint, FaDownload, FaEye,
  FaChevronLeft, FaChevronRight, FaSearch, FaFilter,
  FaTimes, FaFlag, FaComment, FaQuestionCircle, FaBug,
  FaExclamationTriangle, FaMoneyBillWave, FaTrashAlt,
  FaPaperPlane, FaTicketAlt, FaStar, FaGift, FaPlaneDeparture,
  FaIdCard, FaPhoneAlt, FaMapMarkedAlt, FaBirthdayCake
} from 'react-icons/fa';
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (reportPeriod !== 'all') {
      ...
      if (reportPeriod === 'month') {
        ...
        filtered = filtered.filter(r => new Date(r.created_at) > threeMonthsAgo);
      } else if (reportPeriod === '6months') {
        filtered = filtered.filter(r => new Date(r.created_at) > sixMonthsAgo);
      }
    }
  ...
  const handleSubmitReport = async () => {
    ...
    try { 
      ...
      toast.success('Rapport envoyé avec succès. Notre équipe vous répondra dans les plus brefs délais.');
      setReportData({
        subject: '',
        category: 'issue',
        message: '',
  ...
  const handleRequestRefund = async () => {
    ...
    try {
      await axiosInstance.post('/support-service/refund-requests/', {
        ...
        reason: refundReason,
        amount: refundReservation.total_price,
        currency: refundReservation.currency
      });
  ...
  const handleRequestCancellation = async () => {
    ...
    try {
      await axiosInstance.post('/support-service/cancellation-requests/', {
        ...
        reason: cancelReason,
        total_amount: cancelReservation.total_price,
        currency: cancelReservation.currency
      });
  ...
  const exportReservations = () => {
    const dataToExport = filteredReservations.map(r => ({
      ...
      'Statut': r.status,
      'Type de vol': r.trip_type === 'ALLER_SIMPLE' ? 'Aller simple' : 'Aller-retour',
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      'CONFIRMED': { color: 'bg-green-100 text-green-700', icon: FaCheckCircle, label: 'Confirmé' },
      'CANCELLED': { color: 'bg-red-100 text-red-700', icon: FaTimesCircle, label: 'Annulé' },
      'PENDING_PRICE': { color: 'bg-yellow-100 text-yellow-700', icon: FaClock, label: 'En attente' },
      'PRICE_CONFIRMED': { color: 'bg-blue-100 text-blue-700', icon: FaCheckCircle, label: 'Prix confirmé' },
      'FAILED': { color: 'bg-red-100 text-red-700', icon: FaTimesCircle, label: 'Échoué' }
    };
  ...
  const reportCategories = [
    { value: 'issue', label: 'Problème technique', icon: FaBug, color: 'text-red-500' },
    { value: 'complaint', label: 'Réclamation', icon: FaExclamationTriangle, color: 'text-orange-500' },
    { value: 'question', label: 'Question', icon: FaQuestionCircle, color: 'text-blue-500' },
    { value: 'feedback', label: 'Suggestion', icon: FaComment, color: 'text-green-500' },
    { value: 'baggage', label: 'Bagages', icon: FaTicketAlt, color: 'text-purple-500' },
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="w-12 h-12 border-4 border-[${MAIN_COLOR}] border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      {/* Modern Header Design - Not Sticky */}
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex flex-wrap gap-4 justify-center md:justify-start text-white/80 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>{formData.wilaya || 'Wilaya'}, {formData.commune || 'Commune'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <FaBirthdayCake className="w-4 h-4" />
                  <span>{formData.date_naissance ? new Date(formData.date_naissance).toLocaleDateString('fr-FR') : 'Date de naissance'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3 text-center">
                <div className="text-2xl font-bold text-white">
                  {new Date().getFullYear() - (userData?.date_joined ? new Date(userData.date_joined).getFullYear() : 2024)}
                </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {tabs.map(tab => (
            ...
            >
              <tab.icon className={`w-4 h-4 transition-transform group-hover:scale-110 ${activeTab === tab.id ? 'text-white' : 'text-gray-400 group-hover:text-[#00C0E8]'}`} />
              <span>{tab.label}</span>
              {activeTab === tab.id && (
                <motion.div
                  layoutId="activeTab"
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="flex justify-between items-center mb-6 pb-4 border-b">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">Gérez vos informations personnelles et documents de voyage</p>
                </div>
                {!isEditing ? (
                  <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-gray-600 text-sm mb-1">Téléphone *</label>
                  <div className="flex gap-2">
                ...
                <div>
                  <label className="block text-gray-600 text-sm mb-1">Wilaya</label>
                  {isEditing ? (
                    <CustomSelect
                ...
                <div>
                  <label className="block text-gray-600 text-sm mb-1">Commune</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-gray-600 text-sm mb-1">Civilité</label>
                  {isEditing ? (
                    ...
                    <input
                      type="text"
                      value={formData.sexe === 'homme' ? 'Homme' : formData.sexe === 'femme' ? 'Femme' : '-'}
                      disabled
                      className="w-full border border-gray-300 rounded-xl px-4 py-2.5 bg-gray-100"
                    />
                  )}
                ...
                <div>
                  <label className="block text-gray-600 text-sm mb-1">Date de naissance</label>
                  {isEditing ? (
                    <DatePicker
                      value={formData.date_naissance}
                      onChange={(value) => handleInputChange('date_naissance', value)}
                ...
                <div>
                  <label className="block text-gray-600 text-sm mb-1">Numéro de passeport</label>
                  <input
                    type="text"
                    value={formData.num_passport}
                    onChange={(e) => handleInputChange('num_passport', e.target.value)}
                    disabled={!isEditing}
                    className="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:border-[#00C0E8] focus:ring-2 focus:ring-[#00C0E8]/20 disabled:bg-gray-100"
                  />
                </div>
                ...
                <div>
                  <label className="block text-gray-600 text-sm mb-1">Date d'expiration passeport</label>
                  {isEditing ? (
                    <DatePicker
                      value={formData.date_exp_passport}
                      onChange={(value) => handleInputChange('date_exp_passport', value)}
                      placeholder="JJ/MM/AAAA"
                    ...
                    <input
                      type="text"
                      value={formData.date_exp_passport ? new Date(formData.date_exp_passport).toLocaleDateString('fr-FR') : '-'}
                      disabled
                      className="w-full border border-gray-300 rounded-xl px-4 py-2.5 bg-gray-100"
                    />
                  )}
            ...
            >
              <div className="flex justify-between items-center flex-wrap gap-4 mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">Consultez l'historique de vos vols</p>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={exportReservations}
                    className="flex items-center gap-2 px-3 py-2 border rounded-xl hover:bg-gray-50 transition"
                  >
                    ...
                    <div className="flex justify-between items-start flex-wrap gap-2">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-semibold text-gray-800">{reservation.reservation_number}</span>
                          {getStatusBadge(reservation.status)}
                        </div>
                        {reservation.amadeus_pnr && (
                          <div className="text-sm text-gray-500">PNR: {reservation.amadeus_pnr}</div>
                        )}
                      </div>
                    ...
                    <div className="mt-3 pt-3 border-t flex items-center justify-between text-sm">
                      <div className="flex items-center gap-4">
                        <span className="text-gray-600">
                          {reservation.trip_type === 'ALLER_SIMPLE' ? 'Aller simple' : 'Aller-retour'}
                        </span>
                      </div>
                      <div className="flex gap-3">
                        ...
                        <button className="text-[#00C0E8] hover:text-[#0096b8] text-sm font-medium">
                          Voir détails →
                        </button>
                      </div>
                    </div>
          ...
          {activeTab === 'reports' && (
            ...
            >
              <div className="mb-8">
                <h2 className="text-xl font-bold text-gray-800 mb-2">Support client</h2>
                <p className="text-gray-600">
                  Besoin d'aide ? Envoyez-nous un rapport, une réclamation ou une suggestion.
                </p>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div>
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {reportCategories.map((cat) => {
                          ...
                          return (
                            ...
                            >
                              <Icon className={`w-4 h-4 ${reportData.category === cat.value ? 'text-[#00C0E8]' : cat.color}`} />
                    ...
                    <div>
                      <label className="block text-gray-700 text-sm mb-2">Sujet</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-gray-700 text-sm mb-2">Réservation concernée</label>
                      <select
                        value={reportData.reservation_id || ''}
                        onChange={(e) => setReportData(prev => ({ ...prev, reservation_id: e.target.value || null }))}
                      ...
                      >
                        ...
                        {reservations.map((res) => (
                          <option key={res.id} value={res.id}>
                            {res.reservation_number} - {res.total_price} {res.currency}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <button
                      ...
                      disabled={isSubmittingReport || !reportData.message.trim()}
                ...
                <div>
                  ...
                  <div className="bg-blue-50 rounded-xl p-4 mb-4">
                    <h4 className="font-semibold text-blue-800 mb-2">📋 Délais de réponse</h4>
                  ...
                  <div className="bg-green-50 rounded-xl p-4 mb-4">
                    <h4 className="font-semibold text-green-800 mb-2">💰 Remboursements</h4>
                  ...
                  <div className="bg-orange-50 rounded-xl p-4">
                    <h4 className="font-semibold text-orange-800 mb-2">❌ Annulations</h4>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
            ...
            >
              <div className="bg-gradient-to-r from-[#00C0E8] to-[#0096b8] px-6 py-4 flex justify-between items-center">
                <h3 className="text-white font-bold text-lg">Détails de la réservation</h3>
                <button
                  ...
                  <div className="bg-gray-50 rounded-xl p-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{selectedReservation.amadeus_pnr || 'N/A'}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{new Date(selectedReservation.created_at).toLocaleDateString('fr-FR')}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Type de vol</p>
                        <p>{selectedReservation.trip_type === 'ALLER_SIMPLE' ? 'Aller simple' : 'Aller-retour'}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Prix total</p>
                        <p className="font-bold text-[#00C0E8]">{selectedReservation.total_price} {selectedReservation.currency}</p>
                      </div>
                    </div>
                  </div>
                  
                  {selectedReservation.flight_segments && selectedReservation.flight_segments.length > 0 && (
                    <div>
                      <h4 className="font-semibold mb-2">Vols</h4>
                      {selectedReservation.flight_segments.map((segment, idx) => (
                        <div key={idx} className="bg-gray-50 rounded-xl p-3 mb-2">
                          <div className="flex justify-between items-center">
                            <div>
                              <div className="font-semibold">{segment.origin} → {segment.destination}</div>
                              <div className="text-sm text-gray-600">
                                {segment.departure_date} à {segment.departure_time}
                              </div>
                            </div>
                            <div className="text-right">
                              <div className="text-sm font-semibold">{segment.price} {selectedReservation.currency}</div>
                            </div>
                          </div>
                        </div>
                      ))}
            ...
            >
              ...
              <div className="p-6">
                <div className="bg-gray-50 rounded-xl p-4 mb-4">
                  ...
                  <p className="font-semibold">{refundReservation.reservation_number}</p>
                  <p className="text-sm text-gray-600 mt-2">Montant</p>
                  <p className="font-bold text-lg text-orange-600">{refundReservation.total_price} {refundReservation.currency}</p>
                </div>
                
                <div className="mb-4">
                  <label className="block text-gray-700 text-sm mb-2">Raison *</label>
                  <textarea
                    value={refundReason}
      ...
      {/* Cancellation Modal */}
      <AnimatePresence>
        {showCancelModal && cancelReservation && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
            ...
            >
              ...
              <div className="p-6">
                <div className="bg-gray-50 rounded-xl p-4 mb-4">
                  <p className="text-sm text-gray-600">Réservation</p>
                  <p className="font-semibold">{cancelReservation.reservation_number}</p>
                  <p className="text-sm text-gray-600 mt-2">Montant</p>
                  <p className="font-bold text-lg text-red-600">{cancelReservation.total_price} {cancelReservation.currency}</p>
                </div>
